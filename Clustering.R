...
# k-means #
auto.k3m <- kmeans(autox.df, 3, iter.max = 50) #run k-means with number of clusters: 3, maximum iterations:50
auto.k3m
names(auto.k3m) 
auto.k3m$iter
...
autox.scaled.dist <- dist(autox.scaled.df)
...
auto.db3 <- dbscan(autox.scaled.df, eps=0.5,  MinPts = 3) #see help for details
auto.db3
auto.db3$cluster

auto.db7 <- dbscan(autox.scaled.df, eps=0.5,  MinPts = 7) #see help for details
auto.db7
auto.db3$cluster
